        document.addEventListener('DOMContentLoaded', () => {
            ...
            const saveDataToFirestore = async () => {
                ...
                try {
                    ...
                    await docRef.set({ transactions, inExTransactions });
                } catch (error) {
            ...
            const renderSummary = () => {
                ...
                categories.forEach(category => {
                    ...
                    card.innerHTML = `<h3 class="font-semibold text-slate-500">${category}</h3><p class="text-xl font-bold mt-2 text-slate-800">${formatCurrency(total)}</p>`; // Adjusted font size here
            ...
            const inExTableBody = document.getElementById('inExTableBody');
            ...
            const renderInExTransactions = () => {
                ...
                const filteredInExTransactions = inExTransactions.filter(t =>
                    ...
                    t.category.toLowerCase().includes(lowercasedFilter)) &&
                    (inExFilterDate === '' || t.date === inExFilterDate) // Added date filter for IN/EX
                );

                // Sort filtered IN/EX transactions by date (newest first)
                filteredInExTransactions.sort((a, b) => new Date(b.date) - new Date(a.date));

                // Pagination logic for IN/EX
                const inExTotalPages = Math.ceil(filteredInExTransactions.length / inExItemsPerPage);
                const inExStartIndex = (inExCurrentPage - 1) * inExItemsPerPage;
                const inExEndIndex = inExStartIndex + inExItemsPerPage;
                const inExTransactionsToDisplay = filteredInExTransactions.slice(inExStartIndex, inExEndIndex);


                inExTableBody.innerHTML = '';
                inExEmptyState.classList.toggle('hidden', inExTransactionsToDisplay.length > 0);
                inExTransactionsToDisplay.forEach((t) => {
                    const row = document.createElement('tr');
                    row.className = 'bg-white border-b hover:bg-slate-50';
                    const isIncome = t.category === 'IN';
                    row.innerHTML = `<td class="px-4 py-3 whitespace-nowrap">${formatDate(t.date)}</td><td class="px-4 py-3"><span class="px-2 py-1 text-xs font-medium rounded-full ${isIncome ? 'bg-teal-100 text-teal-800' : 'bg-red-100 text-red-800'}">${t.category}</span></td><td class="px-4 py-3 font-medium text-slate-900">${t.detail}</td><td class="px-4 py-3 text-right font-medium ${isIncome ? 'text-teal-600' : 'text-red-600'}">${formatCurrency(t.amount)}</td><td class="px-4 py-3 text-center"><button data-id="${t.id}" class="edit-btn text-sky-500 hover:text-sky-700 mr-3"><i class="fas fa-edit"></i></button><button data-id="${t.id}" class="delete-btn text-red-500 hover:text-red-700"><i class="fas fa-trash-alt"></i></button></td>`;
                    inExTableBody.appendChild(row);
                });
            ...
            inExNextPageBtn.addEventListener('click', () => {
                const inExTotalPages = Math.ceil(inExTransactions.filter(t => {
                    const lowercasedFilter = inExFilterText.toLowerCase();
                    return (t.detail.toLowerCase().includes(lowercasedFilter) ||
                           t.category.toLowerCase().includes(lowercasedFilter)) &&
                           (inExFilterDate === '' || t.date === inExFilterDate);
                }).length / inExItemsPerPage);

                if (inExCurrentPage < inExTotalPages) {
                    inExCurrentPage++;
                    renderInExTransactions();
                }
            });
            ...
            inExTableBody.addEventListener('click', (e) => {
                const editButton = e.target.closest('.edit-btn');
            ...
            downloadAllBtn.addEventListener('click', () => {
                ...
                const allData = { transactions, inExTransactions };
